...
#import "ActiveJobDetailsViewController.h"

@interface ActiveJobDetailsViewController ()
{
...
@implementation ActiveJobDetailsViewController
...
- (void)viewDidLoad {
    ...
    _payOptionView.hidden = YES;
    
    self.automaticallyAdjustsScrollViewInsets = NO;
        
    _payOptionView.layer.borderColor = [Grey_COLOR CGColor];
    _payOptionView.layer.borderWidth = 1.0f;
...
//    _headerView.layer.borderWidth = 1.0f;
    ...
    _payOptionView.layer.cornerRadius = 12;
    _payOptionView.clipsToBounds =YES;
    ...
    if(IS_iPHONE_5){
        ...
        mainScl.contentSize = CGSizeMake(mainScl.frame.size.width, 780);
    }
...
-(IBAction)showProfile:(id)sender
{
    ...
    toilerDetailsVC.toilerDetailDict = [[self.activejobDetailDict valueForKey:@"Bids"] objectAtIndex:0];
...
-(void)setLayout
{
        ...
        [self.profilePic sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",[self.activejobDetailDict valueForKey:@"ToilerPic"]]] placeholderImage:[UIImage imageNamed:@"pipe"] options:0 progress:nil completed:nil];
        self.lblName.text = [self.activejobDetailDict valueForKey:@"ToilerName"];
        self.lblBidAmount.text = [NSString stringWithFormat:@"$%@",[self.activejobDetailDict valueForKey:@"ToilerBidAmount"]];
    ...
    self.lblAmount.text = [NSString stringWithFormat:@"$%@",[self.activejobDetailDict valueForKey:@"Budget"]];
...
- (IBAction)actionCompleteBtn:(UIButton *)sender {
    ...
    UIAlertController * alert=[UIAlertController alertControllerWithTitle:nil
                                                                  message:@"Do you want to mark this job as complete?"
                                                           preferredStyle:UIAlertControllerStyleAlert];
...
- (IBAction)actionChatBtn:(UIButton *)sender {
    ...
    UserChatVC.recieverIDStr = [_activejobDetailDict valueForKey:@"AssignedToilerId"];
...
-(void)FileDispute
{
    ...
    NSMutableDictionary *dict = [[NSMutableDictionary alloc]init];
    [dict setObject:[_activejobDetailDict valueForKey:@"AssignedToilerId"] forKey:@"userid"];
    [dict setObject:[self.activejobDetailDict valueForKey:@"Id"] forKey:@"jobid"];
    ...
    [[NetworkEngine sharedNetworkEngine]MarkJobComplete:^(id object)
     {
         ...
         if ([[object valueForKey:@"message"] isEqualToString:@"success"])
         {
             ...
             if([[NSUserDefaults standardUserDefaults]boolForKey:@"isEmployer"]){
                 _payOptionView.hidden = NO;
                 _blurView.hidden = NO;
