 //
//  CompletedJobDetailsViewController.m
//  Toil Life
//
...
#import "CompletedJobDetailsViewController.h"

@interface CompletedJobDetailsViewController ()
{
...
@implementation CompletedJobDetailsViewController
...
- (void)viewDidLoad {
    ...
    CALayer *imageLayer = _commentView.layer;
    ...
    _commentView.textColor = Grey_COLOR;
...
-(void)setLayout
{
    ...
    self.lblAmount.text = [NSString stringWithFormat:@"$%@",[self.completedJobDict valueForKey:@"CurrentBid"]];
    
    if ([[self.completedJobDict valueForKey:@"IsPaymentRequested"] boolValue]||[[self.completedJobDict valueForKey:@"IsPaymentDone"] boolValue]) {
        [self.requestPaymentBtn setEnabled:NO];
    }
    if ([[self.completedJobDict valueForKey:@"IsDispute"] boolValue]){
    ...
    arrWork = [[self.completedJobDict valueForKey:@"Rating"] mutableCopy];
    [_jobImagesCollectionView reloadData];
    [_commentsTable reloadData];
    [_commentsTable sizeToFit];
...
//        self.commentsTable.translatesAutoresizingMaskIntoConstraints = YES;;
//       self.tableContainerView.frame = CGRectMake( tableframe.origin.x, tableframe.origin.y, tableframe.size.width, tableframe.size.height/4);
    }
...
-(void)RequestPayment
{
    ...
    [[NetworkEngine sharedNetworkEngine]RequestPayment:^(id object)
     {
         NSLog(@"%@",object);
         ...
         {
             [self CreateToken:@"RequestPayment"];
         }
...
-(void)FileDispute
{
    ...
    NSMutableDictionary *dict = [[NSMutableDictionary alloc]init];
    [dict setObject:[_completedJobDict valueForKey:@"UserId"] forKey:@"userid"];
    [dict setObject:[_completedJobDict valueForKey:@"Id"] forKey:@"jobid"];
...
-(void)SaveRating
{
    ...
    [dict setObject:[_completedJobDict valueForKey:@"UserId"] forKey:@"ratetouserid"];
    ...
    [[NetworkEngine sharedNetworkEngine]SaveRating:^(id object)
     {
         ...
         if ([[object valueForKey:@"message"] isEqualToString:@"success"])
         {
             ...
             NSString *dateStr =[NSString stringWithFormat:@"%f",milliseconds];
             
                     [data setObject:dateStr forKey:@"CreatedDate"];
                     [data setObject:ratingStr forKey:@"RateCount"];
                     [data setObject:_txtViewComment.text forKey:@"RateComment"];
    ...
    [[NetworkEngine sharedNetworkEngine]CreateToken:^(id object)
     {
         ...
         if ([[object valueForKey:@"message"] isEqualToString:@"success"])
         {
             ...
             else  if ([type isEqualToString:@"RequestPayment"])
             {
                 [self RequestPayment];
             }
             else
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    NSString *CellIdentifier = @"userCommentCell";
...
- (IBAction)actionBidBtn:(id)sender {
    ...
    {
        UIAlertController* alert = [UIAlertController alertControllerWithTitle:@"Toil.Life"
                                                                       message:@"Please enter your feedback on comment box."
                                                                preferredStyle:UIAlertControllerStyleAlert];
