//
//  EmployerDashboardViewController.m
//  Toil Life
//
...
@interface EmployerDashboardViewController ()
{
...
@implementation EmployerDashboardViewController
...
- (void)viewDidLoad {
    ...
    _toilerNumberView.layer.borderColor = [ORANGE_COLOR CGColor];
    _toilerNumberView.layer.borderWidth = 1.0f;
    _toilerNumberView.layer.cornerRadius = 12;
    //_headerView.layer.cornerRadius = 10;
    _toilerNumberView.clipsToBounds =YES;
...
-(void)filterToilerCategory
{
    ...
    if (arrKeywords.count>0)
    {
        for (NSString *categoryKeyword in arrKeywords)
        {
            ...
            NSPredicate *predicate = [NSPredicate predicateWithFormat:@"Categories CONTAINS[cd] %@",categoryKeyword];
            
            filterdArray = [arrJobData filteredArrayUsingPredicate:predicate];
            
            for (NSMutableDictionary *arr in filterdArray) {
                if(![arrWork containsObject:arr])
                {
                    [arrWork addObject:arr];
                }
...
- (IBAction)actionCreateNewJob:(UIButton *)sender {
    ...
    }else
    {
        ...
        [self.alertEffectView setHidden:NO];
        self.alertTextLable.text =@"You need to register before you can make job posts.";
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([[USERDEFAULTS valueForKey:userData] valueForKey:@"Id"]!=nil)
    {
        ...
        toilerDetailsVC.isFromDashboard = YES;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.lblName.text = [NSString stringWithFormat:@"%@ %@",[dic valueForKey:@"FirstName"],[dic valueForKey:@"LastName"]];
    cell.lblWork.text = [dic valueForKey:@"AboutMe"];
...
- (NSArray *)annotations {
    ...
    for (int valcount=0;valcount<arrWork.count;valcount++)
    {
        ...
        zoomLocation.latitude = [[arrWork[valcount] objectForKey:@"AddressLatitude"] doubleValue];
        zoomLocation.longitude = [[arrWork[valcount] objectForKey:@"AddressLongitude"] doubleValue];
        ...
        if(zoomLocation.latitude != 0 && zoomLocation.longitude != 0)
        {
            ...
            NSArray *arr = [[arrWork[valcount] objectForKey:@"ProfilePic"] componentsSeparatedByString:kBaseMultipartUrl@"/UserImages/"];
            ...
            if (arr.count==1) {
                ...
                {
                NSURL *imageURL = [NSURL URLWithString:[arrWork[valcount] objectForKey:@"ProfilePic"]];
            ...
            //[empire setImage:[UIImage imageNamed:@"pipe"]];
             [empire setImage:[self checkCategories:valcount]];
            empire.title = [NSString stringWithFormat:@"%@ %@",[arrWork[valcount] objectForKey:@"FirstName"],[arrWork[valcount] objectForKey:@"LastName"]];
            
            empire.subtitle = [NSString stringWithFormat:@"%@ KM away",[arrWork[valcount] objectForKey:@"SearchJobRadius"]];
            ...
            empire.disclosureBlock = ^{
                ...
                if ([[USERDEFAULTS valueForKey:userData] valueForKey:@"Id"]!=nil)
                {
                    ToilerDetailsViewController *toilerDetailsVC = [self.storyboard instantiateViewControllerWithIdentifier:@"ToilerDetailsVC"];
                    toilerDetailsVC.isFromDashboard = YES;
                    toilerDetailsVC.toilerDetailDict = [arrWork objectAtIndex:valcount];
                    [self.navigationController pushViewController:toilerDetailsVC animated:YES];
...
{
    UIImage *image;
    NSString *category = [arrWork[index] valueForKey:@"Categories"];
    NSArray *arr = [category componentsSeparatedByString:@","];
    NSString *name = [arr objectAtIndex:0];
    if([name isEqualToString:@"Household Chores"])
    {
        image = [UIImage imageNamed:@"Icon_Chores"];
    }
    else if([name isEqualToString:@"Delivery / Moving Person"])
    {
    ...
    else if([name isEqualToString:@"Trades and Construction"])
    {
        image = [UIImage imageNamed:@"Icon_Construction"];
    }
    else if([name isEqualToString:@"Farm Hand"])
    {
        image = [UIImage imageNamed:@"Icon_Farm"];
    }
    else if([name isEqualToString:@"Services / Rental Industry"])
    {
...
{
    if (arrWork.count>0) {
        [self.noToilerLable setHidden:YES];
    } else {
        [self.noToilerLable setHidden:NO];
    }
    
    [self UnselectAllCategory];
}
...
-(void)GetToilerList
{
    ...
    if ([[USERDEFAULTS valueForKey:userData] valueForKey:@"Id"]!=nil) {
         ...
         [dict setObject:@"0" forKey:@"employerid"];
    ...
    [[NetworkEngine sharedNetworkEngine]GetToilerList:^(id object)
     {
...
-(void)searchTableList:(NSString *)searchtext
{
    if(isSearching)
    {
        NSPredicate *predicate = [NSPredicate predicateWithFormat:@"FirstName CONTAINS[cd] %@ OR LastName CONTAINS[cd] %@ OR Categories CONTAINS[cd] %@ OR AboutMe CONTAINS[cd] %@",searchtext,searchtext,searchtext,searchtext];
